...
2. 开放购票后，**按住 `Ctrl`，用鼠标左键连续点击同一场次的购票链接**，在后台打开多个标签。可以先开 3–5 个。
...
| 问题 | 处理方法 |
| --- | --- |
| 没有面板 | 确认在池袋场次页，检查扩展、脚本及“允许用户脚本”已开启，再刷新 |
| 没有自动选座 | 查看面板提示，确认自动选座已开启且为 12 号厅；加载最多等待 30 秒 |
| 没有合适连座 | 调整人数、区域或高级座位设置，再点击“自动分析并选座” |
| 无法确认场次编号 | 核对场次后，手动点击“自动分析并选座” |
| 想停止或中途失败 | 运行中可点击“停止”；已选座位会保留，重试前请先手动取消 |
| 已有标签完成选座 | 到成功标签继续购票；同场次成功后 15 分钟内禁止重复执行 |
| 页面一直无法识别 | 确认是 IMAX 12 号厅，等待加载后重试；持续异常可 [反馈问题](https://github.com/wjin999/OdyRush/issues) |

## 更新

1. 再次打开 [安装链接](https://raw.githubusercontent.com/wjin999/OdyRush/main/OdyRush.user.js)，按提示更新。
2. 手动安装的用户，可在 Tampermonkey 管理面板打开原脚本，用最新版完整代码替换并保存，避免重复安装。
3. 刷新场次页，重新打开购票标签。

当前版本 **0.6.1**。已通过 29 项自动测试，尚未在真实影院完成占座验证。
